...
class TestNavigation(SeleniumBaseTestCase):
	...
	def test_nav_home(self):
		self.driver.get(self.get_server_url() + url_for('user.retrieve_users'))
		home = self.driver.find_element(By.ID, "home-link")
		home.click()

		# assert that we are in the home page
		self.assertIn(url_for('home'), self.driver.current_url)
		self.assertIn(f"<h1>{TITLE}</h1>", self.driver.page_source)

	@login_user
	def test_nav_logout(self):
		self.driver.get(self.get_server_url() + url_for('user.retrieve_users'))
		logout = self.driver.find_element(By.ID, "logout-link")
		logout.click()
